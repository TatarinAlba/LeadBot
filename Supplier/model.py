from sqlalchemy import create_engine, ForeignKey, Column, Integer, String, TIMESTAMP, text
...
class Account(Base):
    ...
    time_registred = Column(TIMESTAMP(timezone=True), nullable=False, server_default=text('CURRENT_TIMESTAMP'))
...
class Account_by_category(Base):
    ...
    category_id = Column(Integer, ForeignKey('categories.id',ondelete="CASCADE"), primary_key=True, nullable=False, index=True)
    account_telegram_id = Column(Integer, primary_key=True, index=True, nullable=False)
    
    def __init__(self, category_id, account_telegram_id):
